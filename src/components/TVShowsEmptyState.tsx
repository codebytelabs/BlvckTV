...
interface TVShowsEmptyStateProps {
  hasFilters: boolean;
}

export default function TVShowsEmptyState({ hasFilters }: TVShowsEmptyStateProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
      {hasFilters ? (
        ...
        <Tv size={56} className="text-[#6b7280] mb-4" style={{ opacity: 0.4 }} />
      )}
      <p className="text-lg font-semibold text-[#9ca3af]">No TV shows found</p>
      <p className="text-sm text-[#6b7280] mt-2 max-w-sm">
        {hasFilters
          ...
          : tmdbOk
            ...
            : 'Add a valid VITE_TMDB_API_KEY to your .env file to load TV shows from TMDB.'}
      </p>
      {!tmdbOk && (
        <a
