...
import { queryTVCatalog, searchTVShows, GENRES } from '@/hooks/useTMDB';
import PosterCard from '@/components/PosterCard';
import PosterGridSkeleton from '@/components/PosterGridSkeleton';
import TMDBErrorState from '@/components/TMDBErrorState';
import TVShowsEmptyState from '@/components/TVShowsEmptyState';
import CatalogFilterBar from '@/components/CatalogFilterBar';
import { Loader2 } from 'lucide-react';
import type { TVShow, FilterState } from '@/types';
  ...
  const loadPage = useCallback(async (pageNum: number, append: boolean) => {
    ...
    try {
      const result = searchQuery
        ? await searchTVShows(searchQuery, pageNum)
        : await queryTVCatalog(filter, genreId, pageNum);

      setShows(prev => append ? [...prev, ...result.results] : result.results);
      setPage(result.page);
      setTotalPages(result.totalPages);
      setTotalResults(result.totalResults);
    } catch (e) {
      const message = e instanceof Error ? e.message : 'Failed to load TV shows';
      setError(message);
      if (!append) setShows([]);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-6 pb-10 page-fade-in">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-extrabold text-[#f1f1f4]">TV Shows</h1>
        {!loading && !searchQuery && (
      ...
      {loading && shows.length === 0 ? (
        ...
        <TVShowsEmptyState hasFilters={hasActiveFilters} />
      ) : (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-[repeat(auto-fill,minmax(180px,1fr))] gap-3 sm:gap-5">
            {shows.map(show => (
              <PosterCard key={`${show.id}-${show.name}`} item={show} type="tv" />
            ))}
          </div>
